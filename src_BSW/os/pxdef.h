 ...
 *************************************************************************
 *
 *  Copyright 2010 by HighTec EDV Systeme GmbH
 *
 ************************************************************************/
...
#else /* defined (__TC16__) || defined (__TC161__) */
#define PXMEM_ALIGN     4
#define PXMEM_TYPE      PxULong_t
#endif /* defined (__TC16__) || defined (__TC161__) */
...
{ /* 32 bits */
    struct
    {
        unsigned int _res0  :  2;   /* [0:1] reserved */
        unsigned int PROT   :  1;   /* [2] protection compatibility */
        unsigned int RM     :  1;   /* [3] implementation specific */
        unsigned int INT    :  1;   /* [4] implementation specific */
        unsigned int _res1  : 27;   /* [5:31] reserved */
    } bits;
    unsigned long reg;
} __attribute__((aligned(4))) PxCompat_t;
#endif /* __TC16__ */
...
#else
// only ONE core supported for non multicore architectures
#define PX_NO_OF_CORES  1
#define PXCORE_0        0
#define PXCORE_MASTER   PXCORE_0
...
#else
#define TC_NO_OF_DPRS   4 
#define TC_NO_OF_CPRS   2 
#endif
...
typedef struct _PxProtectRegion_T
{
    ...
    PxProtectType_t prot;       // access type to this region
} PxProtectRegion_T;
...
typedef union _PxProtConfReg_T
{
    struct _bits
    {
        unsigned short dp0 : 1;
        unsigned short dp1 : 1;
        unsigned short dp2 : 1;
        unsigned short dp3 : 1;
        unsigned short dp4 : 1;
        unsigned short dp5 : 1;
        unsigned short dp6 : 1;
        unsigned short dp7 : 1;
        unsigned short dp8 : 1;
        unsigned short dp9 : 1;
        unsigned short dp10 : 1;
        unsigned short dp11 : 1;
        unsigned short dp12 : 1;
        unsigned short dp13 : 1;
        unsigned short dp14 : 1;
        unsigned short dp15 : 1;
    } bits;
    unsigned short reg;
} PxProtConfReg_T;
...
/* the task will use Range 1 */
#define RangeSelect     1
typedef union
{
    struct
    {
...
typedef PxProtectMode_T PxCodeProtectMode_T;
...
typedef union
{
    struct
    {
        unsigned int dpm0dbg : 6;
        unsigned int dpm0 : 2;
        unsigned int dpm1dbg : 6;
        unsigned int dpm1 : 2;
        unsigned int dpm2dbg : 6;
        unsigned int dpm2 : 2;
        unsigned int dpm3dbg : 6;
        unsigned int dpm3 : 2;
    } bits;
    unsigned int reg;
} PxProtectMode_T;
...
#define PxMsgIdIsValid(obj) (!((obj).id & _PXIllegalObjId))

#define PxMsgIdInvalidate() ((PxMsg_t){_PXIllegalObjId,PXERR_NOERROR})

#define PxMsgIdSet(id)      ((PxMsg_t){id,PXERR_NOERROR})

#define PxMsgIdGet(obj)     (obj.id)

#define PxMsgIdError(obj)   (obj.error)

#define PxMsgIdErrorSet(obj,err) (obj.error = err)

#define PxMsgId(id, err)    ((PxMsg_t){id,err})

__attribute__ ((always_inline)) static inline PxError_t PxMsgIdResetError(PxMsg_t *obj)
{
    PxError_t err = obj->error;
    obj->error = PXERR_NOERROR;
    return err;
...
#define PxMsgEventEvents(msg)     (msg.events)

#define PxMsgEventEventsSet(msg,ev) (msg.events = ev)

#define PxMsgEventMsg(msg)       (msg.msg)

#define PxMsgEventMsgSet(msg)   (msg.msg = msg)
...
extern "C"
{
    ...
    extern PxError_t PxMsgrelServiceInit(PxMbx_t);
    extern PxMsg_t PxMsgrelService(void);
...
#define PXTmodeDisabledAborts(mode)					\
		(mode & (PXTmodeDisableAborts | PXTmodeTdisableAborts))
#define PXTmodeDisabledTimeslicing(mode)				\
		(mode & (PXTmodeDisableTimeslicing|PXTmodeTdisableTimeslicing))
...
#define __PDP_InitStruct							\
	...
	*/									\
	/*! the protection register definition for of the PXROS system */	\
        ...
        const PxDataProtectSet_T *is_sys_data;                            \
...
/* dummies for initialization extensions */
#define PXInitIntDummies    4
#define PXInitAddrDummies   4
...
/*!
 ...
 tasks; PXMon uses this feature to implement timing profiles for
 task activity
 */
typedef struct
{
...
/*!
 ...
 -  serveral dummies to allow future extensions
...
typedef struct
{
    /*! specification of the system memory class' properties and
     ...
     control block of the initial task and all PXROS objects
     */
    ...
    /*! Specification of the number of PXROS objects (including tasks) and
     ...
     in these versions the size of objects is increased by
     this amount (rounded up to a multiple of "sizeof(PXAligned_t)")
     to make room to store object names. Object names are
     used in monitoring PXROS application systems with the PXROS
     monitor PXMon. Object names larger then the maximal size
     ...
     */
    //lint -e19
    // supress lint warning "useless declaration"
    __PDP_InitStruct;

    //! dummies for future extensions --- should be initialized to "0"
    // PxInt_t      is_int_dummies[PXInitIntDummies];
    //! dummies for future extensions --- should be initialized to "0"
    // PxDataAddr_t is_addr_dummies[PXInitAddrDummies];
...
typedef enum
{
    ...
    PXTraceGetTraceState, /* get actual PXROS trace state         */
    PXTraceGetState, /* get actual PXVIEW trace state        */
    PXTraceSetTraceState, /* set PXROS trace state                */
    PXTraceSetState, /* set PXVIEW trace state               */
    PXTraceGetVersion, /* get PXVIEW version                   */
    PXTraceSetVersion, /* set PXVIEW version                   */
    PXTraceGetTicks, /* get PXVIEW's ticks per second value  */
    PXTraceSetSubticksPerTick, /* set PXVIEW's subticks per tick       */
    PXTraceSetGetSubticksFunc, /* set PXVIEW's function to get subticks*/
    PXTraceStart, /* start PXVIEW trace                   */
    PXTraceStop, /* stop PXVIEW trace                    */
    PXTraceSetTicksPerSecond, /* set PXVIEW's ticks per second value  */
    PXTraceSetPxrosTrace, /* start or stop PXROS trace mechanism  */
    PXTraceEnable, /* enable a PXROS trace group           */
    PXTraceDisable
/* disable a PXROS trace group          */
} PxTraceCtrl_t;
...
extern "C"
{
    ...
    extern PxError_t PxTraceAssignBuffer(PxAligned_t *, PxULong_t);
...
/* we use a special call interface for PxHndcall. The Trampoline function
 ...
 use a special call mechanism. the handler will be called trough a SVC
 and passes the parameters in special registers
 A12 - the function to call normally '___'
 A13 - the static chain to get access to the local variables of the caller
 A15 - the Id of the calling task
 Since the compiler expects the parameter sv in D4 and the taskid in A4
 we have to define the passed taskid as an address
 */
/*!
 \brief _PxHndcall PXROS handler call

 \synopsis _PxHndcall(handler, task, varsize, parms...);

 \param handler function to be called in handlermode
 \param task calling task
 \param varsize total size of the arguments in bytes
 \param parms... parameters to handler
 */
#define _PxHndcall(handler, task, varsize, parms...) (PxArg_t)({		\
  unsigned int *taskId = (unsigned int *) ((unsigned int)(PxTaskIdGet(task)));		\
  __attribute__ ((pxhndcall,noinline)) PxArg_t ___(PxSvc_t sv  __attribute__ ((unused)), unsigned int *tid __attribute__ ((unused))) { /*__i__ = 0;*/return handler(parms); }\
  ___(SV_HNDCALL, taskId);							\
  })
...
typedef enum
{
    _PxNameSrvReqMbxId = 0x4e53524d,
    _PxTcpAccessReqMbxId = 0x5441524d,
    _PxEfsSrvReqMbxId = 0x4546524d,
    _PxNfsSrvReqMbxId = 0x4e46524d,
    _PxMsgrelServiceReqMbxId = 0x526c6e53
} PxMbxReq_t;
 ...
 *************************************************************************
 *
 *  Copyright 2017 by HighTec EDV Systeme GmbH
 *
 ************************************************************************/
...
extern "C"
{
     ...
     * defined in pxinterruptvectortab.cpp
     */
...
#define PXIntMaxIntno		(PxInt_Max_Intrs)
    ...
    {
        PxIntType_t hnd_type; /* type of the interrupt */
        union
        {
            /*
    ...
    void _PxHandleInterrupt(PxUInt_t, PxIntHnd_arg *) __attribute__ ((interrupt_handler));

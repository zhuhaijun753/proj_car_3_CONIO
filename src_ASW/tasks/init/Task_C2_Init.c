...
#pragma section ".Task_C2_Init.stack"
struct
{
	PxStackAligned_t stack[TASK_C2_INIT_STACKSIZE_CORE2];
	PxStackAligned_t intstack[TASK_C2_INIT_INTSTACKSIZE_CORE2];
...
/*******************************************************************************************************
 * Protection regions Task_C2_Init
 *******************************************************************************************************/
...
static const PxTaskContext_T InitTaskContext_CORE2 = {
	.protection[0] =
	{
	...
	.protection[1] =
	{
	    .lowerBound = (PxUInt_t)&Task_C2_Init_obj.stack,
        .upperBound = (PxUInt_t)&Task_C2_Init_obj.stack_protectionPad[0],
        .prot 		= WRProtection
	}
...
/*******************************************************************************************************
 * Init specification Task_C2_Init
 *******************************************************************************************************/

const PxTaskSpec_T InitTaskSpec_CORE2 =
{
		.ts_name = (const PxChar_t *)"Task_C2_Init",
		.ts_fun = Task_C2_Init_Func,
		...
		.ts_context = &InitTaskContext_CORE2,
		.ts_protect_region = InitTaskRegions_CORE2,
		.ts_taskstack =
		{
			.stk_type = PXStackFall,
			.stk_size = PXStackDontCheck,
			.stk_src.stk = &Task_C2_Init_obj.stack[TASK_C2_INIT_STACKSIZE_CORE2]
		},
		.ts_inttaskstack =
		{
			.stk_type = PXStackFall,
			.stk_size = PXStackDontCheck,
			.stk_src.stk = &Task_C2_Init_obj.intstack[TASK_C2_INIT_INTSTACKSIZE_CORE2]
		},
		.ts_abortstacksize = 0
...
/*******************************************************************************************************
 * Task function Task_C2_Init
 *******************************************************************************************************/


void Task_C2_Init_Func(PxTask_t myID, PxMbx_t myMailbox, PxEvents_t myActivationEvents)
{
 ...
 *******************************************************************************************************/
	...
	{
		DET_stop(2,CREATENAMESERVER, result);
	}
#endif
	...
	{
		DET_stop(2,CREATERELEASESERVER, result);
	}
...
/****************  Task_C2_TFT ********************/
    ...
    taskinfo_t Task_C2_TFT_info;

    //Call the task create function
    PxTask_t Task_C2_TFT_Id;
    Task_C2_TFT_Id = Task_C2_TFT_Create(TASK_C2_TFT_PRIO, ev_Task_C2_TFT_0_Activate);

    result = PxTaskIdError(Task_C2_TFT_Id);
    if (result != PXERR_NOERROR)
    {
    ...
    PxMbx_t Task_C2_TFT_Mbx = PxTaskGetMbx(Task_C2_TFT_Id);

    // Register information about Task and Mbx Id ==> the structure of the data depends on the application requirements
    Task_C2_TFT_info.taskId = Task_C2_TFT_Id;
    Task_C2_TFT_info.mbxId = Task_C2_TFT_Mbx;
    ...
    if ((result = PxNameRegister(TASK_C2_TFT_NAMESERVERID,sizeof(taskinfo_t),&Task_C2_TFT_info)) != PXERR_NOERROR)
    {
 ...
 *******************************************************************************************************/
    ...
    if ((result = PxTaskSignalEvents(Task_C2_TFT_Id, ev_Task_C2_TFT_0_Activate)) != PXERR_NOERROR)
	{
 ...
 *******************************************************************************************************/
	...
	//Lowest prio
	PxTaskSetPrio(PxGetId(), MINPRIO_CORE2);
 ...
 *******************************************************************************************************/
	...
	RTE_TIME_appTaskInitDetails[CORE][RTE_TIME_noOfAppTasksInCore1].taskID = Task_C2_TFT_Id;
	PxTaskGetName(Task_C2_TFT_Id, &(RTE_TIME_appTaskInitDetails[CORE][RTE_TIME_noOfAppTasksInCore2++].taskName[0]), 20);
